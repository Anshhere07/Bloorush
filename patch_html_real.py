...
if "tcGateModal" not in html:
    html = html.replace('<!-- JS (IMPORTANT ORDER) -->', tc_modal)
...
if "adminPanelSection" not in html:
    html = html.replace('<!-- Floating View Cart Button (Mobile Only) -->', admin_panel)
...
if "promptAdminLogin" not in html:
    html = re.sub(r'<div class="col-md-3 mb-4">\s*<h5 class="font-weight-bold">Legal</h5>.*?</ul>\s*</div>', footer_admin, html, flags=re.DOTALL)
...
print("HTML in proper Bloorush/Bloorush folder updated!")
